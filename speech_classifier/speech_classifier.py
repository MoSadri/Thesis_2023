...
def process_labeled_data():
    ...
    confusion_output_file = output_directory + f'/original_hate_vs_{data_name}_hate.pdf'
    ...
    output_file = output_directory + f'/original_hate_vs{data_name}_hate.txt'
    with open(output_file, "w") as f:
    ...
    # Now replace all 1 (offensive class) with 0 (hate class), then plot the confusion matrix again
    real_class[real_class == 1] = 0

    confusion_output_file = output_directory + f'/original_hate+offensive_vs_{data_name}_hate.pdf'
    create_confusion_matrix(real_class, predictions, 2, confusion_output_file)

    accuracy = accuracy_score(real_class, predictions)
    precision = precision_score(real_class, predictions, pos_label=0, average=None)
    recall = recall_score(real_class, predictions, pos_label=0, average=None)
    f1 = f1_score(real_class, predictions, pos_label=0, average=None)

    output_file = output_directory + f'/original_hate+offensive_vs_{data_name}_hate.txt'
    with open(output_file, "w") as f:
        f.write(f"Accuracy = {accuracy}\n")
        f.write(f"Precision = {precision}\n")
        f.write(f"Recall = {recall}\n")
...
if __name__ == '__main__':
    ...
    # This is running through a manually labeled data used in the original TDavidson program.
